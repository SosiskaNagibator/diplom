...
function handleAuth($pdo) {
    ...
    if ($action === 'login') {
        ...
        // ---- Администратор (проверка по хешу) ----
        if ($login === 'admin') {
            file_put_contents(__DIR__ . '/auth_debug.log', date('Y-m-d H:i:s') . " Admin login attempt with password: " . $password . "\n", FILE_APPEND);
            ...
            if (!$adminRow) {
                ...
                file_put_contents(__DIR__ . '/auth_debug.log', date('Y-m-d H:i:s') . " Admin was missing, created with hash: " . $hashed . "\n", FILE_APPEND);
            }

            file_put_contents(__DIR__ . '/auth_debug.log', date('Y-m-d H:i:s') . " Password from DB: " . $adminRow['Password'] . "\n", FILE_APPEND);

            if (password_verify($password, $adminRow['Password'])) {
                file_put_contents(__DIR__ . '/auth_debug.log', date('Y-m-d H:i:s') . " Password verified, setting session\n", FILE_APPEND);

                // ---- Устанавливаем сессию и принудительно сохраняем ----
                $_SESSION['is_admin'] = true;
                $_SESSION['user_login'] = 'admin';
                session_regenerate_id(true);  // обновляем ID для безопасности
                session_write_close();         // немедленная запись на диск
                ...
                file_put_contents(__DIR__ . '/auth_debug.log', date('Y-m-d H:i:s') . " Password verification FAILED\n", FILE_APPEND);
