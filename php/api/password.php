...
function handleResetRequest($pdo) {
    ...
    $stmt = $pdo->prepare("SELECT Login FROM users WHERE email = ? LIMIT 1");
    $stmt->execute([$email]);
...
function handleResetConfirm($pdo) {
    ...
    $stmt = $pdo->prepare("SELECT email FROM password_resets WHERE email = ? AND token = ? AND expires_at > NOW()");
    ...
    if ($stmt->execute([$hashedPassword, $user['Login']])) {
        $stmt = $pdo->prepare("DELETE FROM password_resets WHERE email = ?");
        $stmt->execute([$email]);
