...
function Profile() {
  ...
  const fetchBonusHistory = async (login) => {
    try {
      const res = await fetch(`${API_URL}?action=get_bonus_history&login=${encodeURIComponent(login)}`);
      const data = await res.json();
      if (data.status === 'success') {
        setBonusHistory(data.history);
      }
    } catch (err) {
      console.error('Ошибка загрузки истории бонусов:', err);
    }
  };
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {bonusHistory.map((item, index) => (
                    <div key={index} className="flex justify-between items-center py-2 border-b border-gray-50 text-sm">
                      ...
                      <span className={item.amount > 0 ? 'text-green-600 font-semibold' : 'text-red-500 font-semibold'}>
