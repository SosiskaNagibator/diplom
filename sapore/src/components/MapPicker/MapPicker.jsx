    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addressdetails=1`)
      ...
      .then(data => {
        ...
        if (!shortAddr) {
          ...
          setAddress('Адрес без номера дома');
          return;
        }
        setError('');
...
const MapPicker = ({ onAddressSelect, initialAddress }) => {
  ...
  return (
    <div className={styles.container}>
      ...
      {error && (
        <div className="px-3 py-2 text-sm text-red-600 bg-red-50 border-b border-red-200 flex items-center gap-2">
      ...
      <div className={styles.addressLine}>
        {address ? (
          ...
          'Нажмите на карту или введите адрес'
        )}
      </div>
    </div>
  );
